...
def listevents(request):
    ...
    if  request.user.is_authenticated and (request.user.is_staff or  request.user.is_superuser ) :
        tyty=Event.objects.all() #.values('id','event_name' )
        print(tyty)
        #rty = json.loads(str(tyty))
        #print(rty)
        #SomeModel_json = serializers.serialize("json",tyty)
        #print(SomeModel_json)
...
def showallevents(request):
    ...
    serialize = EventSerialize(ev,many = True)
...
@api_view(['GET'])
def show_list_events(request):
    le = ListEvents.objects.all()
...
def post_list_events(request):
    ...
    return Response({'status':200 ,  'payload' : serialize.data,'message':'got the data'})
